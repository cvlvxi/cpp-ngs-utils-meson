...
namespace Intervals {
    ...
    struct Interval {
        ...
        Interval(I a, I b, V val = {}) : low(std::min(a, b)),
                                         high(std::max(a, b)),
                                         value(val) {
        }
        ...
        template<typename I, typename V = ValueType,
                 typename = typename std::enable_if<!std::is_scalar<typename std::decay<I>::type>::value>::type,
                 typename = typename std::enable_if<!std::is_scalar<typename std::decay<V>::type>::value>::type>
        Interval(I &&a, I &&b, V &&val = {}) : low(std::forward<I>(a < b ? a : b)),
        ...
        template<typename V = ValueType,
                 typename = typename std::enable_if<std::is_scalar<typename std::decay<V>::type>::value>::type>
        explicit Interval(const std::tuple<IntervalType, IntervalType> &interval, V val = {}) : Interval(std::get<0>(interval), std::get<1>(interval), val) {
        }


        template<typename V = ValueType,
                 typename = typename std::enable_if<!std::is_scalar<typename std::decay<V>::type>::value>::type>
        explicit Interval(const std::tuple<IntervalType, IntervalType> &interval, V &&val = {}) : Interval(std::get<0>(interval), std::get<1>(interval), std::forward<V>(val)) {
        }
        ...
        bool operator==(const Interval &other) const {
            return !(low < other.low || other.low < low || high < other.high || other.high < high || value < other.value || other.value < value);
        }
    ...
    public:
        ...
        IntervalTree() : m_nill(new Node()),
                         m_root(m_nill) {
        }


        template<typename Container>
        explicit IntervalTree(const Container &intervals) : IntervalTree() {
        ...
        IntervalTree(ForwardIterator begin, const ForwardIterator &end) : IntervalTree() {
            while (begin != end) {
                insert(*begin);
                ++begin;
            }
        ...
        IntervalTree(const IntervalTree &other) : m_nill(new Node()),
                                                  m_root(copySubtree(other.m_root, other.m_nill, m_nill)),
                                                  m_size(other.m_size) {
        }


        IntervalTree(IntervalTree &&other) noexcept : m_nill(other.m_nill),
                                                      m_root(other.m_root),
                                                      m_size(other.m_size) {
    ...
    private:
        ...
        struct Node {
            ...
            Node(Interval interval, Color col, Node *nill) : color(col),
                                                             parent(nill),
                                                             left(nill),
                                                             right(nill),
                                                             high(interval.high),
        ...
        void subtreeOverlappingIntervals(Node *node, const Interval &interval, bool boundary, Callback &&callback) const {
            ...
            if (node->left != m_nill && (boundary ? !(node->left->highest < interval.low) : interval.low < node->left->highest)) {
        ...
        void subtreeInnerIntervals(Node *node, const Interval &interval, bool boundary, Callback &&callback) const {
            ...
            if (node->right != m_nill && (boundary ? !(interval.high < node->right->lowest) : node->right->lowest < interval.high)) {
        ...
        void subtreeOuterIntervals(Node *node, const Interval &interval, bool boundary, Callback &&callback) const {
            ...
            if (node->left != m_nill && (boundary ? !(node->left->highest < interval.high) : interval.high < node->left->highest)) {
